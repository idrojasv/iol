...
class Woo_Checkout_For_Digital_Goods_Admin  {
	...
	<?php	
		 if (!get_option('wcf_plugin_notice_shown')) {
		 	 $current_user = wp_get_current_user();
   			 echo '<div id="wcf_dialog" title="Basic dialog"> <p> Subscribe for latest plugin update and get notified when we update our plugin and launch new products for free! </p> <p><input type="text" id="txt_user_sub_wcf" class="regular-text" name="txt_user_sub_wcf" value="'.$current_user->user_email.'"></p></div>';
  ...
  public function wp_add_plugin_userfn() {
    	...
    	'blocking' => true,
    	'headers' => array(),
    	'body' => array('user'=>array('user_email'=>$email_id,'plugin_site' => $log_url,'status' => 1,'plugin_id' => '4','activation_date'=>$cur_date)),
    	'cookies' => array()));
		update_option('wcf_plugin_notice_shown', 'true');
    }
     public function hide_subscribe_wcffn() {
    	$email_id= $_POST['email_id'];
    	update_option('wcf_plugin_notice_shown', 'true');
    }
